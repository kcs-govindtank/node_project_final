...
const parseNumericFields = (req) => {
    const body = { ...req.body };
    // Parse numeric fields from form-data (which are strings)
    const toNumberIfPresent = (val) => {
    ...
    if (body.id !== undefined)
        body.id = toNumberIfPresent(body.id);
    if (body.categoryId !== undefined)
        body.categoryId = toNumberIfPresent(body.categoryId);
    if (body.subcategoryId !== undefined)
        body.subcategoryId = toNumberIfPresent(body.subcategoryId);
    if (body.countryId !== undefined)
        body.countryId = toNumberIfPresent(body.countryId);
    if (body.stateId !== undefined)
        body.stateId = toNumberIfPresent(body.stateId);
    // Handle file upload
    if (req.file?.path) {
